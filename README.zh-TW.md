...
所有工具都接受可選的 `cookies` 和 `headers` 參數，用於擷取需要登入的頁面。
...
| `/json` | POST | `{url, prompt, schema?, cookies?, headers?, no_cache?}` | 無 | JSON |
...
所有方法都接受 `no_cache=True` 來繞過快取，以及 `cookies`/`headers` 來擷取需要登入的頁面。
