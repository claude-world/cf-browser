...
| `browser_a11y` | url | JSON | 無障礙樹 — LLM 友善的結構化資料 |
...
互動路由（`/click`、`/type`、`/evaluate`、`/interact`、`/submit-form`）需要 `BROWSER` 綁定。未設定時回傳 501。
...
所有方法都接受 `no_cache=True` 來繞過快取、`cookies`/`headers` 來擷取需要登入的頁面、`wait_for` 等待 CSS 選擇器、`wait_until` 設定導航策略（SPA 用 `networkidle0`）、`user_agent` 設定自訂 User-Agent。互動方法在直連模式下會拋出 `NotImplementedError`。
...
- **SSRF 防護**：僅允許 `http://` 和 `https://` URL，阻擋 localhost 和私有 IP 範圍
...
│   │   ├── routes/          15 個端點處理器
│   │   └── lib/             CF API 客戶端、參數映射、快取鍵、URL 驗證
│   ├── tests/
